...
public class ExploreSpring5URLPatternUsingRouterFunctionsIntegrationTest {
  ...
  public void givenRouter_whenGetPathWithSingleCharWildcard_thenGotPathPattern() throws Exception {
    client.get().uri("/paths").exchange().expectStatus().isOk().expectBody(String.class)
        .isEqualTo("/p?ths");
  }

  @Test
  public void givenRouter_whenMultipleURIVariablePattern_thenGotPathVariable() throws Exception {
    client.get().uri("/test/ab/cd").exchange().expectStatus().isOk().expectBody(String.class)
        .isEqualTo("/ab/cd");
  }
  ...
  public void givenRouter_whenGetMultipleCharWildcard_thenGotPathPattern() throws Exception {
    ...
    client.get().uri("/wildcard").exchange().expectStatus().isOk().expectBody(String.class)
  ...
  public void givenRouter_whenGetMultiplePathVaribleInSameSegment_thenGotPathVariables()
    ...
    client.get().uri("/baeldung_tutorial").exchange().expectStatus().isOk().expectBody(String.class)
  ...
  public void givenRouter_whenGetRegexInPathVarible_thenGotPathVariable() throws Exception {
    ...
    client.get().uri("/abcd").exchange().expectStatus().isOk().expectBody(String.class)
        .isEqualTo("/{baeldung:[a-z]+} was accessed and baeldung=abcd");

    client.get().uri("/1234").exchange().expectStatus().is4xxClientError();
  }

  @Test
  public void givenResources_whenAccess_thenGot() throws Exception {
    client.get().uri("/files/test/test.txt").exchange().expectStatus().isOk()
        .expectBody(String.class).isEqualTo("test");

    client.get().uri("/files/hello.txt").exchange().expectStatus().isOk().expectBody(String.class)
        .isEqualTo("hello");
  }
