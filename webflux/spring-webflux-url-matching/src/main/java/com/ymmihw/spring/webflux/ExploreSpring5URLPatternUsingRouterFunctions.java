...
public class ExploreSpring5URLPatternUsingRouterFunctions {
  ...
  private RouterFunction<ServerResponse> routingFunction() {
    ...
    return route(GET("/p?ths"), serverRequest -> ok().body(fromValue("/p?ths")))
        .andRoute(GET("/test/{*id}"),
        ...
        .andRoute(GET("/{var1}_{var2}"),
            serverRequest -> ok().body(fromValue(
                serverRequest.pathVariable("var1") + " , " + serverRequest.pathVariable("var2"))))
        .andRoute(GET("/{baeldung:[a-z]+}"),
            serverRequest -> ok().body(fromValue("/{baeldung:[a-z]+} was accessed and baeldung="
  ...
  WebServer start() throws Exception {
    ...
    HttpHandler httpHandler =
        WebHttpHandlerBuilder.webHandler(webHandler).filter(new IndexRewriteFilter()).build();
