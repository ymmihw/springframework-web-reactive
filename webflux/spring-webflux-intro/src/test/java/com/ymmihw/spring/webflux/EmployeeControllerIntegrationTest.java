...
import com.ymmihw.spring.webflux.EmployeeSpringApplication;
import reactor.core.publisher.Flux;
import reactor.core.publisher.Mono;

@RunWith(SpringRunner.class)
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT,
    classes = EmployeeSpringApplication.class)
...
public class EmployeeControllerIntegrationTest {
  ...
  public void givenEmployeeId_whenGetEmployeeById_thenCorrectEmployee() {
    ...
    testClient.get().uri("/employees/1").exchange().expectStatus().isOk().expectBody(Employee.class)
        .isEqualTo(employee);
  }
  ...
  public void whenGetAllEmployees_thenCorrectEmployees() {
    ...
    testClient.get().uri("/employees").exchange().expectStatus().isOk()
        .expectBodyList(Employee.class).hasSize(3).isEqualTo(employeeList);
  }
}
